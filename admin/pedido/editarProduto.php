...
//`cod_pedido`, `data_pedido`, `data_visita`, `cpf_cliente`, produto, qtde
include_once '../class/pedido.php';
$p = new Pedido();
$id = filter_input(INPUT_GET, 'id');
$p->setCod_pedido($id);
$dados = $p->consultarProduto($id);
foreach ($dados as $mostrar){
    $_codProd = $mostrar['cod_produto'];
    $_qtde = $mostrar['qtde'];
}
?>
            ...
            <form role="form" method="post" enctype="multipart/form-data" name="form" id="form">
                <div class="card-body">
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control"  id="codProduto" name="codProduto" value="<?php echo $_codProd; ?>">
                    </div>
                    <div class="form-group">
                        <label for="qtde">Quantidade do produto</label>
                        <input type="number" class="form-control"  id="qtde" name="qtde" value="<?php echo $_qtde; ?>">
                    </div>                    
                </div>
                <!-- /.card-body -->
...
if (filter_input(INPUT_POST, 'btnSalvar')) {
    if ($p->validarProduto(filter_input(INPUT_POST, 'codProduto', FILTER_SANITIZE_NUMBER_INT))) {
        ...
        . '</div>'
        . '</div>';
        echo '<meta http-equiv="refresh" content="2; url=?p=pedido/consultarProduto"';
    }else{
        echo '<div class="container-fluid">'
